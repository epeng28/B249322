...
```{r message=FALSE, out.height="50%", out.width="75%", fig.cap="Figure 1."}
...
monthly_plot <- salbutamol_monthly %>% 
  ggplot(aes(x = month, y = paid_items_total, group = 1,
             text = paste0(month, "<br>", "Prescriptions: ", paid_items_total))) +  # Add text information for interactive map.
...
```{r, message = FALSE, out.height="30%", out.width="80%", fig.cap="Figure 4. Maps visualising prescription rates across NHS Scotland health boards (left) and average SIMD decile across NHS Scotland health boards(Red- More deprived. Blue- More Affluent) (right). In general, health boards with higher SIMD see lower prescription rates."}
...
# Create function for the maps as multiple will be mapped for visualisation
map_function <- function(fill_change){prescription_mapped %>% 
...
prescription_map <- map_function(prescription_mapped$prescriptions_per_100k) +
...
simd_map <- 
  map_function(prescription_mapped$average_decile) +
